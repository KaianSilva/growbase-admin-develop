	...
	const server = createServer({
		...
		routes() {
			...
			this.post('/cars', (schema, request) => {
				...
				const result = schema.cars.create(attrs);

				return { success: true, data: { car: result }, message: 'Categoria cadastra com sucesso.' };
			});

			this.get('/cars', schema => {
				const result = schema.cars.all();
				console.log(result);
				return { success: true, data: { cars: result.models } };
			});

			this.get('/cars/:id', (schema, request) => {
				const { id } = request.params;

				const result = schema.cars.find(id);
				if (result) {
					return { success: true, data: { car: result } };
				}
				return { success: false, data: 'car NOT FOUD' };
			});
